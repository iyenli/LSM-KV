...
string KVStore::get(uint64_t key)
{
    ...
    if (val == "")
    {
        ...
        int first_level_amount = cacheptr.size();
        for (int i = 0; i < first_level_amount; ++i)
        {
            // in search mode: big stamp in the front
            for (auto iter = cacheptr[i].begin(); iter != cacheptr[i].end(); ++iter)
            {
                // check if the key is in the range of the sstablecache
                if (key <= (*iter)->max_val && key >= (*iter)->min_val)
                {
                    int index = (*iter)->get_offset(key);
                    if (index < 0)
                        continue;

                    string path = (*iter)->dir + (*iter)->generate_level() +
                                  (*iter)->generate_name();
                    ifstream fs(path, ios::binary);

                    uint32_t offset = (*iter)->key_offset[index].offset;
                    fs.seekg(offset);

                    // next NULL: to tail, else compute len
                    if (index == (*iter)->amount - 1)
                        fs >> val;
                    else
                    {
                        uint32_t nextOffset = ((*iter)->key_offset)[index + 1].offset;
